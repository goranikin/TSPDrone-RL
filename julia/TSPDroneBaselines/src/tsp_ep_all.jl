...
function find_tsp_tour(x::Vector{Float64}, y::Vector{Float64})
    scale_factor = 1000
    tsp_tour, _ = Concorde.solve_tsp(x * scale_factor, y * scale_factor; dist="EUC_2D")
    @assert tsp_tour[1] == 1
    return tsp_tour
end
            ...
            for k_tour in 2:(n - 1)
                ...
                for i_node in precomputed_order[k_node, :]
                    ...
                    while j_tour !== n + 1
                        ...
                        t_cost =
                            sum_Ct[i_tour, k_tour - 1] +
                            Ct[r[k_tour - 1], r[k_tour + 1]] +
                            sum_Ct[k_tour + 1, j_tour]
                        cost = max(t_cost, d_cost)
...
function tsp_ep_all(
    ...
    Ct, Cd = cost_matrices_with_dummy(
        x_coordinates,
        y_coordinates,
        truck_cost_factor,
        drone_cost_factor,
    ...
    return tsp_ep_all(
        ...
        quadratic_ep_boost=quadratic_ep_boost,
    )
end
...
function tsp_ep_all(
    ...
    improved = true
    precomputed_order =
        (quadratic_ep_boost && flying_range != MAX_DRONE_RANGE) ?
        precompute_dist_order(Cd) : nothing
    best_obj, t_route, d_route = exact_partitioning(
        init_tour,
        Ct,
        Cd;
        flying_range=flying_range,
        ...
        for i in 2:(n - 1)
            ...
            for local_search in local_search_methods
                for j in 2:(n - 1)
                    ...
                    if is_valid
                        ep_time, t_route, d_route = exact_partitioning(
                            new_tour,
                            Ct,
                            Cd;
                            flying_range=flying_range,
