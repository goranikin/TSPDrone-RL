...
class Env:
    ...
    def step(self, idx_truck, idx_drone, time_vec_truck, time_vec_drone, terminated):
        ...
        b = np.equal(
            (
                self.combined_nodes[np.arange(self.batch_size), self.truck_loc] == 1
            ).astype(int)
            + a.astype(int),
            2,
        )
        ...
        a = np.equal(
            np.equal(time_vec_drone[:, 1], 0).astype(int)
            + np.equal(self.sortie, 1).astype(int)
            + np.equal(self.returned, 1).astype(int),
            3,
        )
        b_s = np.where(a)[0]
        avail_actions[b_s, :, 1] += (
            (self.combined_nodes[b_s] - self.combined_check[b_s])
            * np.expand_dims(np.greater(self.state[b_s].sum(axis=1), 2).astype(int), 1)
            * np.ones([len(b_s), self.n_nodes])
        )
